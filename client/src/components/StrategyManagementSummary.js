...
export default function StrategyManagementSummary({ autoRiskManagement, setAutoRiskManagement, RRR, setRRR, lotSize, setLotSize, signalCooldown, setSignalCooldown }) {
  ...
  const getSignalCooldownItems = () => {
    ...
    return items.map((n) => (<MenuItem key={uuid()} value={n.toString()}>{n + (n===0?" candle":" candles")}</MenuItem>))
  }
    ...
    <React.Fragment>
      <div className={styles.padding}>
        ...
        <FormControl fullWidth className={styles.margin}>
          <InputLabel className={styles.labelPadding}>Trading Strategy</InputLabel>
          <Select fullWidth defaultValue={"0"} variant="filled">
            <MenuItem value={"0"}>
              Open a new position anytime a signal occurs and close all open positions of inverse direction
            </MenuItem>
            {/* <MenuItem value={"1"}>
        ...
        <FormControl fullWidth className={styles.margin}>
          <InputLabel className={styles.labelPadding}>Signal Cooldown</InputLabel>
          <Select
            fullWidth
          ...
          >
            <MenuItem key={uuid()} value={"0"}>No cooldown</MenuItem>
            { 
